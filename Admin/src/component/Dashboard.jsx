...
function Dashboard() {
  ...
  const fetchEmotions = async () => {
    try {
      const response = await fetch(`http://${import.meta.env.VITE_SERVER_IP}:3001/emotion_data`, {
        method: 'POST',
        headers: {
  ...
  const fetchCountMembers = async () => {
    try {
      const response = await fetch(`http://${import.meta.env.VITE_SERVER_IP}:3001/CountMembers`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const fetchGender = async () => {
    try {
      const response = await fetch(`http://${import.meta.env.VITE_SERVER_IP}:3001/DashBoardGender`, {
        method: 'Post',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleSubmit = async (event) => {
    try {
      ...
      // Send API request
      const response = await fetch(`http://${import.meta.env.VITE_SERVER_IP}:3001/send_email`, {
        method: 'POST',
        body: formData,
      });
